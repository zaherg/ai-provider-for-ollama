...
### Changed
- Updated provider metadata to classify Ollama as a cloud provider so it can appear in WP AI Client credential UI flows that only list cloud providers.
...
- Optional Ollama API key support (`OLLAMA_API_KEY`) for secured/proxied or cloud deployments.
...
- Added Ollama Cloud API access notes and guidance for exact deployment base URLs.
